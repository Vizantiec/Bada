...
/** 
 * @file		FBaseErrors.h 
 * @brief		This header file defines the Osp framework's error codes.
 *
...
/** Thrown when the connection is busy and therefore cannot process a new request. */
...
/** Thrown when the device is processing the previous task and therefore cannot process a new request. */
#define E_DEVICE_BUSY					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1403))

/** Thrown when the device fails with an unknown reason. */
#define E_DEVICE_FAILED					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1404))

/** Thrown when the device does not support a specific request. */
...
#define E_DNS							(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1408))

/** Thrown when the DNS cannot resolve the requested address. */
#define E_DNS_NOT_FOUND					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1409))
...
/** Thrown when the end of file or end of stream is reached unexpectedly
 * during an input operation. */
#define E_END_OF_FILE					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1503))


//----------------------------------------------------------------------------F
/* Thrown when the requested operation fails due to an undefined reason. */
//#define E_FAILURE						(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1600))

/** (specialized) Thrown when the application tries to call with a number that is
 * not allowed in the FDN mode, while the FDN mode is enabled. */
#define E_FDN_MODE						(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1601))

/** Thrown when an attempt to create the file denoted by the specified pathname
 *  fails. */
#define E_FILE_ALREADY_EXIST			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1602))

/** Thrown when an attempt to open the file denoted by a specified pathname
 *  fails. */
#define E_FILE_NOT_FOUND				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1603))
...
/** (specialized) ... */
#define E_NO_CERTIFICATE				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1803))

/** The socket connection is closed by user. */
#define E_SOCKET_USER					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(1804))
...
/** Thrown when a specified library not exists. */
 ...
 * user is not. */
#define E_NOT_A_MEMBER					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2402))

/** (specialized) Thrown when Bluetooth pairing is not established. */
#define E_NOT_PAIRED					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2403))

/** Thrown when the target is not responding. */
#define E_NOT_RESPONDING				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2404))

/** Thrown when the specified string does not represent valid number. */
#define E_NUM_FORMAT					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2405))

/** Thrown when the URL is changed. */
#define E_URL_CHANGED					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2406))
 ...
 * initializing. */
#define E_ON_INITIALIZING				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2502))

/** Thrown when the operation is canceled explicitly. */
#define E_OPERATION_CANCELED			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2503))

/** Thrown when the operation fails due to certain reason. */
#define E_OPERATION_FAILED				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2504))

/** Thrown when the memory is not sufficient to perform the requested
 * operation. */
#define E_OUT_OF_MEMORY					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2505))

/** Thrown when the internal state of the current instance reaches the
 * valid range. */
#define E_OUT_OF_RANGE					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2506))

/** Thrown when the operation has caused an overflow. */
#define E_OVERFLOW						(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2507))
...
/** Thrown when the paring fails due to any reason. */
...
/** Thrown when a server tells the device that operation has failed due to some
...
#define E_STORAGE_FULL					(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2910))

/** Thrown when the specified symbol is not found. */
#define E_SYMBOL_NOT_FOUND				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2911))

/** Thrown when the input statement does not confirm to the specific syntax. */
#define E_SYNTAX						(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2912))

/** Thrown when a failure occurs from the underlying system. */
#define E_SYSTEM						(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2913))

/** Thrown when a service is deactivated. */
#define E_SERVICE_DEACTIVATED			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(2914))
...
#define E_USER_AGENT_NOT_ALLOWED		(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3111))

/** Thrown when a user is already registered to the device. */
#define E_USER_ALREADY_REGISTERED		(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3112))

/** Thrown when the required user does not exist. */
#define E_USER_NOT_FOUND				(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3113))

/** Thrown when no user is registered to the device yet. */
#define E_USER_NOT_REGISTERED			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3114))

/** Thrown when the specified language is not supported. */
#define E_UNSUPPORTED_LANGUAGE 			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3115))

/** Thrown when the specified version of the input content is not supported. */
#define E_UNSUPPORTED_VERSION			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3116))

/** Thrown when the specified locale is not supported. */
#define E_UNSUPPORTED_LOCALE 			(ERR_SRC_FRAMEWORK + SET_E_CAUSE(3117))

/** Thrown when the dimension has changed */
#define E_DIMENSION_CHANGED                    (ERR_SRC_FRAMEWORK + SET_E_CAUSE(3118))


//----------------------------------------------------------------------------V

//----------------------------------------------------------------------------W
/** (specialized) Thrown when non-blocking socket operation cannot be
